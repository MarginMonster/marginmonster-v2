...
/** Animate a portrait into a lip-synced talking video. image/audio are data
 *  URIs (fal accepts them in the URL fields). Uses the async queue API since
 *  video gen is slow; polls up to ~10 min. Returns the output video URL, or
 *  throws so the pipeline falls through to omni-human/kling. */
export async function animateAvatar(imageDataUri: string, audioDataUri: string): Promise<string> {
  ...
  for (let i = 0; i < 120; i++) {
    ...
    if (sj.status === "FAILED" || sj.status === "ERROR") throw new Error(`fal status ${sj.status}`);
